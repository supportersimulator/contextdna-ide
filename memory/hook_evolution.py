...
# Use same database as pattern evolution (consolidation, not fragmentation)
    ...
    "UserPromptSubmit": {
        ...
        "risk_keywords": {
            "critical": [
                ...
                "destroy", "delete.*all", "drop.*table", "drop.*database", "truncate",
                "rm\\s+-rf", "force.*push", "reset.*hard", "wipe", "purge", "nuke",
                # Production operations
                "migration.*prod", "prod.*migration", "schema.*change", "alter.*table",
                "force.*push.*main", "force.*push.*master", "rollback.*prod",
                # Security operations
                "auth.*system", "credentials.*rotate", "secret.*rotation", "root.*access",
                "master.*key", "encryption.*key",
                # Data operations
                "backup.*delete", "restore.*prod", "data.*migration.*prod"
            ],
            "high": [
                # Infrastructure changes
                "deploy", "terraform", "pulumi", "cloudformation", "kubernetes", "k8s",
                "helm", "ecs.*service", "eks", "fargate", "lambda.*deploy", "cdk.*deploy",
                # Production operations
                "production", "live", "main.*branch", "master.*branch", "release", "hotfix",
                # Database operations
                "migration", "migrate", "schema", "database", "rds", "dynamodb",
                "postgres", "mysql", "mongodb", "redis.*cluster",
                # Security operations
                "security", "ssl", "cert", "tls", "credentials", "secrets", "api.*key",
                "access.*key", "token", "oauth", "jwt", "password", "iam", "firewall",
                # Scaling/infrastructure
                "scale", "autoscal", "load.*balanc", "cdn", "dns", "route53",
                # AWS services
                "aws", "ec2", "s3.*bucket", "sqs", "sns",
                # Refactoring
                "refactor", "restructur", "rewrite", "overhaul"
            ],
            "moderate": [
                # Container operations
                "docker", "container", "compose", "dockerfile", "image.*build",
                # Configuration
                "config", "env", "environment", "settings", "parameter",
                # Health/monitoring
                "toggle", "feature.*flag", "health", "monitor", "metric", "logging",
                # Sync/update operations
                "sync", "update", "upgrade", "patch", "change", "modify",
                # Bug fixes
                "fix", "bug", "issue", "error", "debug",
                # Testing
                "test.*env", "staging", "uat", "qa",
                # Backup/restore
                "backup", "snapshot"
            ],
            "low": [
                # UI/display
                "admin", "dashboard", "display", "show", "view", "add.*button",
                "ui", "ux", "frontend",
                # Styling
                "style", "css", "theme", "color", "font", "layout",
                # Documentation
                "format", "comment", "readme", "doc", "changelog",
                # Code quality
                "lint", "prettier", "formatting", "typo", "rename", "cleanup",
                # Local development
...
class HookEvolutionEngine:
    ...
    def _init_db(self) -> sqlite3.Connection:
        ...
        db = connect_wal(EVOLUTION_DB, check_same_thread=False)
        ...
        # A/B config versions table — snapshot/rollback for autonomous tests
        db.execute("""
            CREATE TABLE IF NOT EXISTS ab_config_versions (
                ...
                revert_time REAL,
                FOREIGN KEY (test_id) REFERENCES hook_ab_tests(test_id)
            )
        """)

        # Add autonomous A/B columns to hook_ab_tests if missing
        _auto_cols = [
            ("consensus_json", "TEXT"),
            ("pre_auth_rules", "TEXT"),
            ("grace_until", "REAL"),
            ("auto_revert_triggered", "INTEGER DEFAULT 0"),
            ("degradation_metrics", "TEXT"),
            ("hypothesis", "TEXT"),
            ("max_duration_hours", "REAL DEFAULT 48"),
        ]
        for col_name, col_type in _auto_cols:
            try:
                db.execute(f"ALTER TABLE hook_ab_tests ADD COLUMN {col_name} {col_type}")
            except sqlite3.OperationalError:
                pass  # Column already exists
        ...
        db.execute("CREATE INDEX IF NOT EXISTS idx_ab_config_versions_test ON ab_config_versions(test_id)")
    ...
    def record_hook_fire(self, hook_type: str, session_id: str, variant_id: str,
                         risk_level: str = "", prompt_hash: str = ""):
        """Record that a hook variant was fired for a session.

        This creates a pending outcome record. The actual outcome (positive/negative)
        will be recorded later by success_detection or failure_pattern_analyzer.
        """
        try:
            with sqlite3.connect(str(self.db_path)) as conn:
                conn.execute("""
                    INSERT OR IGNORE INTO hook_fires (
                        fire_id, hook_type, session_id, variant_id,
                        risk_level, prompt_hash, fired_at, outcome
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    f"{session_id}_{prompt_hash}_{int(time.time())}",
                    hook_type, session_id, variant_id,
                    risk_level, prompt_hash,
    ...
    def resolve_pending_outcomes(self, outcome: str = "positive"):
        """Resolve all pending hook fires from the last 5 minutes with the given outcome."""
        try:
            with sqlite3.connect(str(self.db_path)) as conn:
                conn.execute("""
                    UPDATE hook_fires
    ...
    def record_outcome(self, variant_id: str, session_id: str,
        ...
        try:
            ...
            # Gap 6: Propagate variant outcome to observability store
            try:
                from memory.observability_store import get_observability_store
                obs = get_observability_store()
                # Find the experiment this variant belongs to
                exp_row = self.db.execute(
                    "SELECT experiment_id FROM hook_experiments WHERE control_variant_id = ? OR winner_variant_id = ?",
                    (variant_id, variant_id)
                ).fetchone()
                if exp_row:
                    obs.record_variant_outcome(
                        injection_id=f"hook_{session_id}",
                        experiment_id=exp_row["experiment_id"],
                        variant_id=variant_id,
                        outcome_success=(outcome == "positive"),
                        outcome_reward=confidence if outcome == "positive" else -confidence,
                    )
            except Exception:
                pass  # Non-critical: observability store may not be ready
