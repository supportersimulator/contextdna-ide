...
def detect_sop_type(combined: str) -> str:
    ...
    if re.search(r'\b(is|are|will be)\s+(another|one)\s+option\b', combined_lower):
        has_growth = True  # Present/future indefinite = growth

    # === DECIDE with potential length override ===
    # If strong growth signals, override to process (even if has problem words)
    if has_growth and not has_finality:
        return 'process'

    # If strong finality signals, confirm/override to bugfix
...
class ArtifactDeduplicator:
    ...
    def register_artifact(self, filename: str, path: str, content: str):
        ...
        try:
            ...
            print(f"[WARN] Artifact hash storage failed: {e}")
...
def generate_bugfix_sop_title_with_status(task: str, details: str = None) -> Optional[str]:
    """
    Generate bugfix SOP title WITH status prefix.

    Extends generate_bugfix_sop_title() by:
    1. Generating the standard 6-zone title
    2. Adding (passed MM/DD/YY) status prefix
    3. Recording the bugfix in tracker

    Output format:
    (passed 01/23/26) [bug-fix SOP] HEART: bad_sign → (antecedent) → fix → (stack) → outcome

    Args:
        task: Original task/error description
        details: Additional details for zone extraction

    Returns:
        Bugfix SOP title with status prefix, or None if content is process
    """
    # Generate standard title first
    title = generate_bugfix_sop_title(task, details)

    if title is None:
        return None  # Not a bugfix SOP

    # Add status tracking
    if BUGFIX_TRACKER_AVAILABLE:
        # Extract symptom and fix from details if possible
        symptom = task[:100] if task else ""
        fix = ""
        if details:
            # Try to extract the fix from details
            fix_patterns = [
                r'fix(?:ed)?:?\s*(.{10,80})',
                r'solution:?\s*(.{10,80})',
                r'resolved by:?\s*(.{10,80})',
                r'→\s*([^→]+)$'  # Last arrow segment is often the fix
            ]
        ...
        # Record and get formatted title
        return add_status_to_new_bugfix(title, symptom, fix)
    else:
        # No tracker - just return title without status
        return title


def mark_bugfix_passed(title: str, symptom: str = "", fix: str = "") -> str:
    """
    Mark a bugfix as currently passing.

    Call this when a fix is verified working.

    Args:
        title: The bugfix SOP title
        symptom: The original symptom (optional)
        fix: The fix that was applied (optional)
    ...
    record_bugfix_passed(title, symptom, fix)
    return format_bugfix_with_status(title)


def mark_bugfix_failed(title: str, failure_note: str = "") -> Tuple[str, bool, str]:
    """
    Mark a bugfix as failing (regression).

    Call this when a fix stops working.
    ...
    Returns:
        ...
        - formatted_title: Title with (failed MM/DD/YY) prefix
        - is_regression: True if this was previously passing
        - alert_message: Alert to show user if regression detected
    """
    if not BUGFIX_TRACKER_AVAILABLE:
        return (title, False, "")
...
def get_bugfixes_needing_attention() -> List[Dict]:
    """
    Get all bugfixes that are currently failing and need attention.

    Returns:
        List of bugfix entries with status='failed'
    """
    if not BUGFIX_TRACKER_AVAILABLE:
        return []
...
def check_symptom_for_regression(symptom: str) -> Optional[Tuple[Dict, str]]:
    """
    Check if a symptom matches a known bugfix (potential regression).

    Call this when an error occurs to detect regressions.

    Args:
        symptom: The error/symptom text

    Returns:
        Tuple of (bugfix_entry, alert_message) if match found, None otherwise
    """
    if not BUGFIX_TRACKER_AVAILABLE:
        return None
