"""
RACE S2 — Tests for S8 Redis cache layer in synaptic_deep_voice.

S8 (synaptic_8th_intelligence) is the LAST major LLM call in the webhook
injection pipeline (120s timeout — Synaptic deep dive). This cache mirrors
R1 (S2 wisdom) and M3 (S6 guidance) and gives us identical guarantees.
...
Verifies:
  ...
  - Cache key isolates prompt content
  - TTL expiry path -> miss again
  - Redis down -> graceful fallthrough to LLM (ZSF: error counted, not silent)
  - S8_CACHE_ENABLED=0 disables the cache path entirely
  - Public API of generate_deep_s8 is unchanged
...
# =========================================================================
# Fake Redis with TTL semantics + crash mode (mirrors race/m3 + race/r1 infra)
# =========================================================================
...
def deep_voice(monkeypatch):
    ...
    monkeypatch.setattr(dv, "_get_subconscious_patterns", lambda: "pattern: late-night async")
    monkeypatch.setattr(dv, "_get_emotional_awareness", lambda: "emotional: focused")
    ...
    def _mock_llm_generate(**kw):
        ...
        # Return >20 chars so generate_deep_s8 keeps the result
        return f"Aaron, subconscious voice #{calls['n']}: I notice you keep circling this."
...
def cache_on(monkeypatch):
    """Force S8_CACHE_ENABLED=1 regardless of operator env."""
    monkeypatch.setenv("S8_CACHE_ENABLED", "1")
    # Short TTL by default — individual tests may override.
    monkeypatch.setenv("S8_CACHE_TTL_S", "600")
...
def test_first_call_misses_then_hits_llm(deep_voice, fake_redis, cache_on):
    ...
    dv, calls = deep_voice
    result = dv.generate_deep_s8("fix webhook timeouts", session_id="s1")

    assert result is not None
    assert calls["n"] == 1, "LLM should be called exactly once on a cold call"

    stats = dv.get_s8_cache_stats()
    assert stats["s8_cache_misses"] == 1
    assert stats["s8_cache_hits"] == 0
    assert stats["s8_cache_errors"] == 0
...
def test_second_call_same_inputs_hits_cache(deep_voice, fake_redis, cache_on):
    ...
    dv, calls = deep_voice
    first = dv.generate_deep_s8("fix webhook timeouts", session_id="s1")
    assert calls["n"] == 1

    second = dv.generate_deep_s8("fix webhook timeouts", session_id="s1")
    assert second == first, "Cache hit must return the exact stored value"
    assert calls["n"] == 1, "LLM must NOT be called on a cache hit"

    stats = dv.get_s8_cache_stats()
    assert stats["s8_cache_misses"] == 1
    assert stats["s8_cache_hits"] == 1
...
def test_cache_key_isolates_sessions(deep_voice, fake_redis, cache_on):
    ...
    dv, calls = deep_voice
    dv.generate_deep_s8("same prompt", session_id="alice")
    dv.generate_deep_s8("same prompt", session_id="bob")

    assert calls["n"] == 2, "Different sessions must each invoke the LLM"
    stats = dv.get_s8_cache_stats()
    assert stats["s8_cache_misses"] == 2
    assert stats["s8_cache_hits"] == 0


def test_cache_key_isolates_prompt(deep_voice, fake_redis, cache_on):
    """Different prompts -> distinct cache entries."""
    dv, calls = deep_voice
    dv.generate_deep_s8("prompt A", session_id="s1")
    dv.generate_deep_s8("prompt B", session_id="s1")
    assert calls["n"] == 2
...
def test_ttl_expiry_misses_again(deep_voice, fake_redis, cache_on, monkeypatch):
    ...
    dv, calls = deep_voice
    monkeypatch.setenv("S8_CACHE_TTL_S", "1")  # 1 second TTL

    dv.generate_deep_s8("p", session_id="s1")
    assert calls["n"] == 1
...
def test_redis_down_fallthrough_to_llm(deep_voice, fake_redis, cache_on):
    """ZSF: if Redis raises on every op, generate_deep_s8 still returns LLM output
    and bumps the s8_cache_errors counter — never silent."""
    dv, calls = deep_voice
    fake_redis.fail_mode = True

    result = dv.generate_deep_s8("p", session_id="s1")
    ...
    dv.generate_deep_s8("p2", session_id="s1")
    stats = dv.get_s8_cache_stats()
    # At minimum we must have logged either an error or a miss for each path.
    assert (stats["s8_cache_errors"] + stats["s8_cache_misses"]) >= 1


def test_cache_disabled_via_env(deep_voice, fake_redis, monkeypatch):
    """S8_CACHE_ENABLED=0 -> cache layer is bypassed entirely, every call hits LLM."""
    dv, calls = deep_voice
    monkeypatch.setenv("S8_CACHE_ENABLED", "0")
    ...
    stats = dv.get_s8_cache_stats()
    assert stats["s8_cache_hits"] == 0
    assert stats["s8_cache_misses"] == 0


def test_public_api_unchanged(deep_voice, fake_redis, cache_on):
    """generate_deep_s8 keeps the same signature: (prompt, session_id)."""
    import inspect
    dv, _calls = deep_voice
    sig = inspect.signature(dv.generate_deep_s8)
    assert list(sig.parameters.keys()) == ["prompt", "session_id"]
