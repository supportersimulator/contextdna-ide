...
# Evolution database - resolve symlinks for Docker compatibility
EVOLUTION_DB = _resolve_db_path(Path(__file__).parent / ".pattern_evolution.db")
...
class PatternEvolutionEngine:
    ...
    def _promote_candidates(self) -> List[str]:
        ...
        for row in cursor.fetchall():
            ...
            # Add to pattern registry via public API
            if self.registry:
                try:
                    from memory.pattern_registry import LearnedPattern
                    new_pattern = LearnedPattern(
                        id=None,
                        pattern=regex,
                        confidence=confidence,
                        evidence_type=category,
                        source_task=f"evolved (occurrences: {occurrences})",
                        times_matched=occurrences,
                        times_confirmed=0,
                        created_at=__import__('datetime').datetime.now().isoformat(),
                        last_matched=__import__('datetime').datetime.now().isoformat(),
                    )
                    self.registry.db.add_pattern(new_pattern)
    ...
    def record_feedback(self, regex: str, was_false_positive: bool, context: str = ""):
        ...
        try:
            ...
            print(f"[WARN] Pattern feedback recording failed: {e}")
    ...
    def cache_risk_analysis(self, regex: str, risk_score: float, risk_level: str, issues: List[str]):
        ...
        try:
            ...
            print(f"[WARN] Pattern risk cache write failed: {e}")
