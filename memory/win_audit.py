...
class WinAuditSystem:
    ...
    def _load_cache(self) -> Dict:
        ...
        if AUDIT_CACHE_FILE.exists():
            try:
                ...
                print(f"[WARN] Audit cache load failed: {e}")
    ...
    def _get_captured_successes(self, hours: int) -> List[CapturedSuccess]:
        ...
        # Get from Context DNA if available
        if CONTEXT_DNA_AVAILABLE:
            try:
                memory = ContextDNAClient()
                # Query recent wins from Context DNA
                ...
                print(f"[WARN] Acontext win lookup failed: {e}")
...
if __name__ == "__main__":
    ...
    if cmd == "audit":
        ...
        print(f"  Context DNA: {'Available' if CONTEXT_DNA_AVAILABLE else 'Not available'}")
