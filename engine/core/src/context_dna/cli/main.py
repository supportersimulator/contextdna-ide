...
Usage:
    ...
    context-dna doctor                  Run health checks and diagnostics
    ...
    AUTH:
    context-dna auth status             Show device auth status
    context-dna auth login              Register device with backend
    context-dna auth logout             Unlink device from backend
...
def cmd_auth(args, brain: Brain) -> int:
    """Manage device authentication and backend linking."""
    import json
    import platform
    ...
    API_BASE = "https://api.ersimulator.com"

    def get_device_info():
        """Get current device info from file."""
    ...
    def detect_platform():
        """Detect platform for device metadata."""
    ...
    def register_with_backend(device_id: str, user_email: str = None):
        """Register device with Django backend."""
        data = {
            "device_token": device_id,
            "platform": detect_platform(),
            "device_name": platform.node(),
            "app_version": "cli-0.2.0",
        }

        try:
            req = urllib.request.Request(
                f"{API_BASE}/api/auth/device/register/",
                data=json.dumps(data).encode('utf-8'),
                headers={
                    'Content-Type': 'application/json',
                    'Origin': 'https://contextdna.io',  # Triggers Context DNA user linking
                },
                method='POST',
            )
    ...
    if args.auth_action == "status":
        ...
        if not device_info:
            ...
            print()
            print("Run: context-dna auth login")
            return 0

        print("Device Authentication Status")
        print("=" * 40)
        print(f"Device ID:    {device_info.get('device_id', 'unknown')[:8]}...")
        print(f"Platform:     {device_info.get('platform', 'unknown')}")
        print(f"Created:      {device_info.get('created_at', 'unknown')[:10]}")

        if device_info.get('linked_to_backend'):
            print(f"Backend:      ✓ Linked")
            print(f"User:         {device_info.get('user_email', 'unknown')}")
            print(f"Linked at:    {device_info.get('linked_at', 'unknown')[:10]}")
        else:
            print(f"Backend:      Not linked")
            print()
            print("Run: context-dna auth login")
        ...
        # Register/link device with backend
        print("Registering device with Context DNA backend...")
        print()
        ...
        if device_info and device_info.get('device_id'):
            ...
            print(f"Using existing device ID: {device_id[:8]}...")
        else:
            device_id = str(uuid_mod.uuid4())
            print(f"Generated new device ID: {device_id[:8]}...")
        ...
        import os
        user_email = os.environ.get('CONTEXT_DNA_USER_EMAIL', user_email)

        if user_email:
            print(f"User email: {user_email}")
        else:
            print("Warning: No user email configured")
            print("Set in ~/.context-dna/config.yaml or CONTEXT_DNA_USER_EMAIL env var")

        # Register with backend
        print()
        print("Connecting to backend...")
        result = register_with_backend(device_id, user_email)

        if result:
            # Update device info
            device_data = {
                'device_id': device_id,
                'device_token': result.get('device_token', device_id),
                'user_id': result.get('user_id'),
                'created_at': device_info.get('created_at') if device_info else datetime.now(timezone.utc).isoformat(),
                'platform': detect_platform(),
                'user_email': user_email or '',
                'linked_to_backend': True,
            ...
            print(f"  Device Token: {result.get('device_token', device_id)[:8]}...")
            print(f"  User ID: {result.get('user_id')}")
            print(f"  New: {'Yes' if result.get('created') else 'No'}")
            return 0
        else:
            print()
        ...
        # Clear device linkage (keep device ID)
        device_info = get_device_info()
        if not device_info:
            print("Not logged in")
            return 0

        device_info['linked_to_backend'] = False
        device_info.pop('linked_at', None)
        device_info.pop('user_id', None)
        device_info.pop('device_token', None)
        save_device_info(device_info)

        print("✓ Device unlinked from backend")
        print("  Device ID preserved for future login")
        return 0

    else:
        print("Usage:")
        print("  context-dna auth status   Show authentication status")
        print("  context-dna auth login    Register device with backend")
        print("  context-dna auth logout   Unlink device from backend")
        return 1
...
def cmd_doctor(args, brain: Brain) -> int:
    """Run health checks and diagnostics."""
    from context_dna.doctor import run_doctor

    output_format = "json" if args.json else "auto"
    return run_doctor(
        project_dir=brain.project_dir,
...
def cmd_auto_learn(args, brain: Brain) -> int:
    ...
    try:
        ...
        # Silently fail in hook, but log for debugging
        pass
...
def main() -> int:
    ...
    p_init.add_argument("--backend", choices=["sqlite", "context-dna-api"], default="sqlite")
    ...
    # doctor
    p_doctor = subparsers.add_parser("doctor", help="Run health checks and diagnostics")
    p_doctor.add_argument("--json", action="store_true", help="Output as JSON")
    p_doctor.add_argument("--fix", action="store_true", help="Attempt to fix issues")
    ...
    # auth
    p_auth = subparsers.add_parser("auth", help="Manage device authentication")
