...
# =============================================================================
# Backend Adapter for API compatibility
# =============================================================================

class _AcontextBackendAdapter:
    """Adapter that wraps Acontext queries as a storage backend."""

    def __init__(self, brain):
        self._brain = brain

    def list_learnings(self, limit: int = 100, offset: int = 0, learning_type: str = None):
        """List learnings from Acontext SOPs."""
        from context_dna.storage.backend import Learning, LearningType
        from datetime import datetime

        results = search_sops("")  # Get all
        learnings = []

        for r in results[:limit]:
            learning = Learning(
                id=r.get("id", str(hash(r.get("when", "")))[:8]),
                type=LearningType.SOP,
                title=r.get("title", r.get("when", "")),
                content=r.get("key", r.get("content", "")),
                tags=r.get("tags", []),
...
# =============================================================================
# Brain Wrapper - CLI-compatible interface
# =============================================================================

class Brain:
    """
    CLI-compatible wrapper around ArchitectureBrain.

    Provides the interface expected by context_dna.cli.main with proper
    Learning and LearningType support.
    """
    ...
    def __init__(self, project_dir: str = None):
        ...
        # Try to use storage backend if available
        self._storage = None
        try:
            from context_dna.storage.sqlite import SQLiteBackend
            storage_path = self._project_dir / ".context-dna" / "learnings.db"
            if storage_path.parent.exists():
                self._storage = SQLiteBackend(str(storage_path))
        except Exception:
            pass
    ...
    @property
    def backend(self):
        """Storage backend for API compatibility."""
        if self._storage:
            return self._storage
        # Return a minimal backend that wraps Acontext queries
        return _AcontextBackendAdapter(self)

    def init(self, backend: str = "local", project_name: str = None) -> dict:
        """Initialize Context DNA for a project."""
        config_dir = self._project_dir / ".context-dna"
        config_dir.mkdir(parents=True, exist_ok=True)

        self._brain.init_session()

        # Initialize storage backend
        if backend == "local" and self._storage is None:
            try:
                from context_dna.storage.sqlite import SQLiteBackend
                storage_path = config_dir / "learnings.db"
                self._storage = SQLiteBackend(str(storage_path))
            except Exception:
                pass

        return {
            "project": project_name or self._project_dir.name,
            "storage": {"backend": backend},
            "initialized": True
        }

    def win(self, title: str, details: str = None, tags: list = None) -> str:
        """Record a win/success."""
        learning_id = str(self._uuid.uuid4())[:8]

        # Record via ArchitectureBrain
        area = tags[0] if tags else None
        self._brain.capture_win(title, details, area=area)

        # Also record in local storage if available
        if self._storage:
            learning = self._Learning(
                id=learning_id,
                type=self._LearningType.WIN,
                title=title,
                content=details or "",
                tags=tags or [],
            )
            self._storage.record(learning)

        return learning_id

    def fix(self, problem: str, solution: str, tags: list = None) -> str:
        """Record a fix/gotcha."""
        learning_id = str(self._uuid.uuid4())[:8]

        # Record via ArchitectureBrain
        self._brain.capture_fix(problem, solution)

        # Also record in local storage if available
        if self._storage:
            learning = self._Learning(
                id=learning_id,
                type=self._LearningType.FIX,
                title=problem,
                content=solution,
                tags=tags or [],
            )
    ...
    def pattern(self, name: str, description: str = None, tags: list = None) -> str:
        """Record a pattern."""
        learning_id = str(self._uuid.uuid4())[:8]
        ...
        if self._storage:
            learning = self._Learning(
                id=learning_id,
                type=self._LearningType.PATTERN,
                title=name,
    ...
    def sop(self, title: str, steps: str = None, tags: list = None) -> str:
        """Record a Standard Operating Procedure."""
        learning_id = str(self._uuid.uuid4())[:8]
    ...
    def query(self, search: str, limit: int = 10, learning_type=None):
        """Query memory for learnings."""
        # Try local storage first
        if self._storage:
            try:
                return self._storage.query(search, limit=limit, learning_type=learning_type)
            except Exception:
                pass

        # Fall back to Acontext SOPs
        results = search_sops(search)
        learnings = []
        for r in results[:limit]:
            learning = self._Learning(
                id=str(self._uuid.uuid4())[:8],
                type=self._LearningType.SOP,
                title=r.get("title", r.get("when", "")),
                content=r.get("key", r.get("content", "")),
                tags=r.get("tags", []),
    ...
    def consult(self, task_description: str) -> str:
        """Get wisdom before starting work (alias for context)."""
        return self._brain.get_context_for_task(task_description)

    def context(self, task_description: str) -> str:
        """Get context for a task."""
        return self._brain.get_context_for_task(task_description)

    def recent(self, hours: int = 24, limit: int = 20):
        """Get recent learnings."""
        if self._storage:
            try:
                return self._storage.get_recent(hours=hours, limit=limit)
            except Exception:
                pass
        return []
    ...
    def status(self) -> dict:
        ...
        stats = {
            # Required by CLI
            "project": self._project_dir.name,
            "backend": "sqlite" if self._storage else "context-dna-api",
            "healthy": True,
            "total": storage_stats.get("total", self._brain.cache.get("successes_captured", 0)),
            "today": storage_stats.get("today", 0),
            "last_capture": self._brain.cache.get("last_cycle"),
            "by_type": storage_stats.get("by_type", {}),

            # Extended info
            "cycles_run": self._brain.cache.get("cycles_run", 0),
            "successes_captured": self._brain.cache.get("successes_captured", 0),
            "patterns_discovered": len(self._brain.cache.get("patterns_ever_detected", [])),
            "components": {
                "context_dna_api": CONTEXT_DNA_AVAILABLE,
                "auto_capture": AUTO_CAPTURE_AVAILABLE,
                "enhancer": ENHANCER_AVAILABLE,
                "context": CONTEXT_AVAILABLE,
                "knowledge_graph": KNOWLEDGE_GRAPH_AVAILABLE,
                "objective_success": OBJECTIVE_SUCCESS_AVAILABLE,
                "sop_types": SOP_TYPES_AVAILABLE,
            }
        }

        return stats

    def export_learnings(self) -> list:
        """Export all learnings as list of dicts."""
        if self._storage:
            try:
                learnings = self._storage.get_recent(hours=24*365, limit=10000)
                return [l.to_dict() for l in learnings]
            except Exception:
                pass
        return []

    def import_learnings(self, learnings: list) -> int:
        """Import learnings from list of dicts. Returns count imported."""
        ...
        for data in learnings:
            try:
                learning = self._Learning.from_dict(data)
                self._storage.record(learning)
                count += 1
            except Exception:
                pass
