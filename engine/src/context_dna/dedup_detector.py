...
try:
    ...
    DB_PATH = get_db_path()
    REGISTRY_PATH = get_user_data_dir() / '.pattern_registry.db'
except ImportError:
...
class DuplicateDetector:
    ...
    def __init__(self):
        self.evolution_db = sqlite3.connect(str(DB_PATH))
        self.evolution_db.row_factory = sqlite3.Row

        self.registry_db = None
        if REGISTRY_PATH.exists():
            self.registry_db = sqlite3.connect(str(REGISTRY_PATH))
            self.registry_db.row_factory = sqlite3.Row
